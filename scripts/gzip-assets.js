...
async function gzipFile(filePath) {
  ...
  try {
    ...
    if (stats.size < MIN_SIZE_BYTES) {
      console.log(`⏭️  Skipping ${filePath} (${stats.size} bytes < ${MIN_SIZE_BYTES} threshold)`);
      return;
    }
    ...
    const reduction = ((stats.size - gzipStats.size) / stats.size * 100).toFixed(1);
    
    console.log(`✅ ${path.relative(process.cwd(), filePath)}`);
    console.log(`   ${stats.size.toLocaleString()} → ${gzipStats.size.toLocaleString()} bytes (${reduction}% reduction)`);
    
    return { original: stats.size, compressed: gzipStats.size, reduction: parseFloat(reduction) };
  } catch (error) {
...
async function main() {
  ...
  if (results.length > 0) {
    ...
    console.log(`   Compressed size: ${totalCompressed.toLocaleString()} bytes`);
    console.log(`   Total savings: ${(totalOriginal - totalCompressed).toLocaleString()} bytes`);
