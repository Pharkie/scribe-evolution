(()=>{function i(){return{method:"GET",path:"/unknown",showContent:!1,init(){this.extractErrorDetails(),setTimeout(()=>{this.showContent=!0},100),this.startFloatingAnimation()},extractErrorDetails(){let e=document.querySelector("[data-error-method]"),t=document.querySelector("[data-error-path]");if(e){let r=e.textContent||e.getAttribute("data-error-method");r&&r!=="{{METHOD}}"&&(this.method=r)}if(t){let r=t.textContent||t.getAttribute("data-error-path");r&&r!=="{{URI}}"&&(this.path=r)}let o=new URLSearchParams(window.location.search);o.get("method")&&(this.method=o.get("method")),o.get("path")&&(this.path=o.get("path"))},get errorMessage(){return this.path.includes("api")?"That API endpoint seems to have gotten lost in the paper feed.":this.path.includes("css")||this.path.includes("js")?"That resource got caught in the ribbon cartridge.":this.path.includes("image")||this.path.includes("img")?"That image fell behind the thermal head.":"Looks like that page got tangled up in the rollers."},get suggestions(){let e=["Try the main printer interface","Check system diagnostics"];return this.path.includes("settings")?e.unshift("Go to settings configuration"):this.path.includes("api")?e.unshift("Review API documentation"):this.path.includes("diagnostics")&&e.unshift("Access system diagnostics"),e},get errorIconClass(){return""},goHome(){window.location.href="/"},goToSettings(){window.location.href="/settings/"},goToDiagnostics(){window.location.href="/diagnostics.html"},goBack(){window.goBack()},tryAgain(){window.location.reload()},startFloatingAnimation(){if(window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;if(setInterval(()=>{let t=document.createElement("div");t.className="floating-paper",t.style.cssText=`
                    position: fixed;
                    top: -50px;
